...
### 文件编码强制要求
- **所有**生成的代码文件、脚本、配置文件必须严格使用 **UTF-8 (无 BOM)** 编码。
- 禁止使用 GBK、GB2312、Latin-1 或系统默认编码。
- 当脚本中包含中文注释、字符串或路径时，必须在文件头部显式声明编码（Python 的 `# -*- coding: utf-8 -*-`）。

### 脚本编写规范
- **Python 脚本**：文件首行必须是 `#!/usr/bin/env python3`，第二行必须是 `# -*- coding: utf-8 -*-`。
- **Shell/Bash 脚本**：确保 `echo` 或变量赋值中的中文不会被截断，建议使用 `$'...'` 语法或确保终端环境为 UTF-8。

### 输出与日志
- 控制台输出涉及中文时，不要假设终端支持特定编码，建议在脚本开头添加环境检查或编码设置代码。
- Python 日志输出到文件时，使用 `encoding='utf-8'` 参数打开文件句柄。
- Windows 环境下若控制台乱码，优先在脚本中设置 `sys.stdout.reconfigure(encoding='utf-8')`。
...
核心文件：`test_cases.py`（200条黄金用例）、`metrics.py`（轨道/意图/槽位指标）、`runner.py`（离线评估）、`telemetry.py`（在线计数）、`llm_judge.py`（LLM-as-Judge）、`regression_check.py`（CI回归）。
